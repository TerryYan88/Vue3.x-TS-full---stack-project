  ...
  <div class="detail" v-if="rst">
    ...
    <div class="detail_desc">
      ...
      <div class="desc_coupon" v-if="redpack.length">
        <div class="coupon_main" @click="show.coupon = true">
          <div
            class="coupon_item flex type0"
            v-for="item in redpack"
            :key="item.id"
          >
      ...
      >
        ...
        <div class="coupon_item flex type0" v-for="item in redpack" :key='item.id'>
          <!-- {{redpack}} -->
          <span class="mian_value">{{ item.value }}</span>
          <span class="mian_detail">{{ item.title }}</span>
          <span class="mian_btn">领取</span>
    ...
    <div class="detail_main">
      ...
      <nav class="main_nav flex" v-fixed   ref="detail_nav">
        <div class="nav_item">
          <router-link :to="_paths.menu">点餐</router-link>
        </div>
        <div class="nav_item">
          <router-link :to="_paths.evaluate">评价</router-link>
        </div>
        <div class="nav_item">
          <router-link :to="_paths.merchant">商家</router-link>
        </div>
      </nav>
